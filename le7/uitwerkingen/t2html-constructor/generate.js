...
/**
 * @function Element
 * @desc     Een constructor voor Element objecten
 * @param  {string} tagname - HDe tag van het element
 * @param  {string} content - De content van het element
 * @return {object} EenElement object
 */
...
function labelParagraphs(pars) {
  ...
  function label(par) {
    ...
    if (index > 0) {
      res = new Element(H + index, par.slice(index + 1));
    } 
    else {
      res = new Element(P, labelContent(par.slice(index)));
    }
    return res;
  }
